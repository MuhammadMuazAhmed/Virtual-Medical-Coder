...
export async function PUT(req: NextRequest, { params }: { params: { code: string } }) {
    ...
    try {
        ...
        const idx = arr.findIndex((e) => String(e.cpt) === String(code));
        ...
        console.error("CPT PUT ERROR:", error);
        return NextResponse.json({ error: "Failed to update CPT entry" }, { status: 500 });
    }
}
...
export async function DELETE(req: NextRequest, { params }: { params: { code: string } }) {
    ...
    try {
        ...
        const newArr = arr.filter((e) => String(e.cpt) !== String(code));
        ...
        console.error("CPT DELETE ERROR:", error);
        return NextResponse.json({ error: "Failed to delete CPT entry" }, { status: 500 });
    }
}
